...
5. Common project tool usage: The test suite can be run with `uv run pytest -v`, code coverage reports can be generated by running the command `uv run slipcover -m pytest -v`, lint checking can be done with `ruff check . --fix; pyright .; ruff format .`, and formatting - by itself - can be done with `ruff format .`.
...
8. When significant changes are made to functions/classes `ruff check . --fix; pyright .; ruff format .` should be run from the project folder to ensure there are no simple syntax or lint errors/warnings.
